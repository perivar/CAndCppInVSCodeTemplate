...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
	...
	{
        double volumeCooked = pow(20.0, volume_dB/20.0);
       
        // --- output silence: change this with your signal render code
        if (noteOn)
        {
            double trivialSaw = unipolarToBipolar(moduloCounter);
            
            processFrameInfo.audioOutputFrame[0] = volumeCooked * trivialSaw;
            if (processFrameInfo.channelIOConfig.outputChannelFormat == kCFStereo)
                processFrameInfo.audioOutputFrame[1] = volumeCooked * trivialSaw;
            ...
            if (moduloCounter >= 1.0)
                moduloCounter -= (int)moduloCounter;
...
bool PluginCore::processMIDIEvent(midiEvent& event)
{
    ...
    {
        double oscillatorFrequency = midiFreqTable[event.midiData1];
        phaseInc = oscillatorFrequency / audioProcDescriptor.sampleRate;
        moduloCounter = 0.0;
